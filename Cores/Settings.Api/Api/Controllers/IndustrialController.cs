namespace ProlecGE.ControlPisoMX.Cores.Testing.Settings.Api.Controllers
{
    ...
    [ApiController]
    [Route("api/v1/industrial")]
    public class IndustrialController : ControllerBase
    {
        #region Fields

        private readonly ILogger<IndustrialController> logger;
        ...
        public IndustrialController(ILogger<IndustrialController> logger, IMediator mediator)
        {
            this.logger = logger;
        ...
        public async Task<ActionResult<string>> MissingItemDesignContact()
        {
            try
            {
                ...
                string contact = await mediator
                    .Send(new IndustrialMissingCoreDesignContactQuery(), CancellationToken.None)
        ...
        public async Task<ActionResult<string>> MissingCoreManufacturingPlanContact()
        {
            try
            {
                ...
                string contact = await mediator
                    .Send(new IndustrialMissingCoreManufacturingPlanContactQuery(), CancellationToken.None)
                    .ConfigureAwait(false);
                return Ok(contact);
            }
            catch (Exception ex)
        ...
        public async Task<ActionResult<string>> InternalErrorContact()
        {
            try
            {
                ...
                string contact = await mediator
                    .Send(new IndustrialInternalErrorContactQuery(), CancellationToken.None)
                    .ConfigureAwait(false);
